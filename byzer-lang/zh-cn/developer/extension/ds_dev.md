...
Kolo具备足够灵活的扩展性，能够同时解决 Data + AI 领域的问题。我们提供了大量的插件，方便用户在数据处理、商业分析和机器学习的不同场景中使用 Kolo。这些插件类型包括: DataSource、ET、Script、App，我们都可以灵活的通过离线或者线上的方式注册到 Kolo Engine 中使用。



在 Kolo 中，ET（Estimator/Transformer的简称）是一个非常重要的概念。通过 ET，我们可以完成非常多的复杂任务。包括：
...
ET 也是实现将算法的特征工程从训练复用到预测时的核心，即大部分特征工程训练完成后都可以转化为一个函数，从而供给预测时使用。比如 `train,register,predict` 语法去抽象机器学习流程，通过 ET 一站式解决模型的训练、注册、预测的语法定制。本节，我们会以表抽取插件为例，来介绍如何用包装 Spark 内置的工具应用 于 Kolo 中。
...
下面我们将重点介绍第一种方式——直接修改 Kolo 源码的方式添加 ET 插件。
...
最后，将开发好的 ET 注册到 ETRegister 中，启动我们的 Kolo Engine 即可使用了。下面我们来一起看一下如何实现一个 ET，用于抽取 SQL 语句中所有的表名功能。

### Class definition

首先，介绍一个目前 Kolo 里已经提供了一个工具类，可以让我们复用 Spark SQL 解析器来进行解析抽取表名：
...
我们看到，train 其实接受了很多参数。这些参数都是 train 里的 params 传递的。我们看到 params 的签名是 `Map[String, String]` ， 所以在 Kolo 中，所有的属性配置都是字符串。我们先对方法做个解释：
...
with VersionCompatibility // 兼容哪些版本的Kolo
```

我们可以使用 Kolo 已经支持的解析器 `MLSQLAuthParser` ，下面是 train 部分所有的代码：
...
// 指定该插件能够兼容哪些版本的Kolo。Kolo会在加载插件的时候会通过该检查兼容性。[插件兼容性版本](https://github.com/allwefantasy/mlsql/issues/1544)支持指定版本和指定范围2中方式，如下版本表达式 [2.0.0,) 表示大于或等于 2.0.0
...
在上面的示例中，参数的定义还是比较复杂的，我们也非常建议完成的定义该数据结构，因为在代码自动补全、workflow中插件可视化、了解参数原信息和参数间依赖关系等方面都非常有用。如需了解更多参数机制，请访问[开发插件自省参数](?)



### **ET组件的权限**

> Kolo 将一切资源都抽象成了表，最细粒度可以控制到列级别。

在 Kolo 中有非常完善的权限体系，我们可以轻松控制任何数据源到列级别的访问权限，而且创新性的提出了编译时权限，也就是通过静态分析 Kolo 脚本从而完成表级别权限的校验（列级别依然需要运行时完成）。

Kolo 的权限内置于语言级别。这意味着，通过 Kolo 我们就可以控制各种数据源的访问，亦或是Kolo 自身的各种功能，我们会在后面的内容具体阐述如何开发Kolo的权限体系。下面用一个图更好的描述：

![img13.png](images/img13.png)

**基本描述如下：**

1.  Kolo Engine接收脚本，检查是否开启授权验证
...
3.  失败，则 Kolo Engine 会拒绝执行该脚本。
4.  成功，则 Kolo Engine 会继续执行该脚本。
...
在auth的实现中，我们发现在条件 `extractTables` 中定义了一个 `MLSQLTable` ，表示Kolo解析脚本后返回的表的所有信息，其对应的数据结构如下：
...
然后我们在Kolo Engine启动项配置好请求权限服务的AuthClient客户端，这里演示Kolo内置的client：
...
在Kolo中也支持列级别的控制，如果需要了解更多Kolo权限相关的内容，参考：[编译时权限控制](../../../../public/blog_archive/Compile-time_access_control.md)



### 注册到Kolo引擎

到目前为止，我们就实现了一个抽取表名称的 ET 插件了。那么如何注册到 Kolo 引擎中呢？如果是作为内置插件，我们只要添加如下一行代码到`tech.mlsql.ets.register.ETRegister`即可：

```Scala
register("SyntaxAnalyzeExt", "tech.mlsql.plugins.ets.SyntaxAnalyzeExt")
```

如果是一个外部插件，我们可以通过网络安装的方式或者离线安装方式，以 jar 的方式添加到 Kolo 引擎中，具体实现方法在本文下面的小节 `作为外置插件使用` 会有介绍。

现在，你启动 IDE，就可以使用这个模块了。



我们开发好的 ET 可以很简单的封装为宏命令，简化交互上的使用。比如 ET `SQLShowTableExt`，就是我们常用的命令 !`desc`，我们可以参考文章: [命令行开发](./et_command.md)
...
我们以`RandomForest`为例，看下一个模型怎么在Kolo中使用。
...
-- 一旦设置为 true，每次运行此脚本时，Kolo 都会为您的模型生成新目录
...
上面我们介绍了直接修改 Kolo 源码的方式添加 ET，如果你希望这个插件是一个独立的模块，并且内置在 Kolo 中，那么你需要在 external 目录下新建一个模块，然后在`tech.mlsql.runtime.PluginHook` 添加该内置插件。另外，你还需要在 streamingpro-mlsql 添加该该模块依赖。通常添加在profile/streamingpro-spark-2.4.0-adaptor 和 profile/streamingpro-spark-3.0.0-adaptor 中都要添加。如果你这个模块只兼容其中一个，添加一个即可。



## 作为外置插件使用

如果你想作为外置插件使用，也就是单独做成一个项目开发和维护，可以参考项目: [kolo-extension](https://github.com/byzer-org/kolo-extension)

模式和内置插件一样，然后打成jar包，使用[离线安装的方式安装](../../extension/installation/offline_install.md)，即手动下载好jar包放置到程序目录，并在启动命令中设置 jar 包以及启动类。MLSQL 外置插件可以动态安装，但是如果要更新，则需要重启服务。

我们也支持[网络安装插件](../../extension/installation/online_install.md)，直接使用命令行方式在 Console 里安装。比如，如果需要安装 excel 支持，一行命令在 MLSQL Console 里即可搞定：
