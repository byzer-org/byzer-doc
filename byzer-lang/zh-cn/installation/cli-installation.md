...
- **PATH**：指定一个路径列表，用于搜索可执行文件。执行一个可执行文件时，如果该文件不能在当前路径下找到，则会依次寻找 `PATH` 中的每一个路径，直至找到可执行文件为止。Kolo-lang 命令行的执行命令 (kolo) 在其安装路径下的 bin 目录中。因此我们应该将该路径添加到 PATH 变量中。

2. 下载 kolo-lang 命令行

下载 kolo-lang 命令行程序包，地址如下：

- [Mac 版](http://download.mlsql.tech/kolo-lang-mac-amd64-2.3.0-preview.tar.gz)
- [Linux版](http://download.mlsql.tech/kolo-lang-linux-amd64-2.3.0-preview.tar.gz)

3. 目录结构

本示例以 mac 环境为例，下载 [适配 mac 环境的压缩包](http://download.mlsql.tech/kolo-lang-mac-amd64-2.3.0-preview.tar.gz) 后，对 tar 包进行解压缩，内部的目录结构如下：

```
./kolo-lang-mac-amd64-2.3.0-preview
|-- bin
|-- jdk8
|-- libs
|-- logs
...
修改完环境变量后，可以通过 `version` 验证是否安装成功和当前 kolo 命令行版本，具体命令如下：
