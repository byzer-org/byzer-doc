...
在 Kolo-lang 中，表的生命周期是 session 级别的。这意味着当一个表被使用过后，系统会自动记住。这样可以很方便的用户进行调试。 但是当使用 `!if/!else` 的时候则会有困惑发生。
...
当一次运行的时候，系统会报错，因为没有表 `b`。 如果将 `2==1` 修改为 `1==1` 时，则系统正常运行。 如果将 `1==1` 再次修改为 `2==1` 此时系统依然正常运行。 
